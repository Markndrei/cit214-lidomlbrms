...
const BlogDetailPage: FC<BlogDetailPageProps> = async ({ params }) => {
  ...
  const lib =
    book?.library === 1
      ? "Iloilo Provincial Library and Archives"
      : "Ilolo City Public Library";

  return (
    <div>
      <BackButton />
      <a className="relative block overflow-hidden rounded-lg border border-gray-100 p-4 sm:p-6 lg:p-8 mt-10">
        <span className="absolute inset-x-0 bottom-0 h-2 bg-gradient-to-r from-slate-900 via-slate-600 to-slate-300"></span>
        ...
        <div className="absolute left-10 top-40">
          <div className="mt-4">
            <p className="text-pretty text-sm text-gray-500">Library: {lib}</p>
      ...
      <br />
      <ButtonAction id={String(params.id)} />
    </div>
  );
};
